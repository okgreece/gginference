...
#' @param colaccept color the acceptance area of the test, see  
#' \code{\link{colors}}
...
ggvartest <- function(t,
  ...
  ggplot(data = data.frame(points), aes(points)) +
    ...
    geom_vline(data = data.frame(x = t$statistic),
               aes(xintercept = data.frame(x = t$statistic)$x),
               colour = colstat,
               lwd = 1) +
    geom_text(data = data.frame(x = t$statistic),
              aes(x = data.frame(x = t$statistic)$x,
                  y = 0,
                  label = paste("test statistic=",
                                round(data.frame(x = t$statistic)$x, 3))),
              size = 4,
              angle = 90,
              vjust = -0.4,
              hjust = 0,
